...
<p align="center">
<img src="pipeline.png" alt="SE-CCM Pipeline" width="700">
</p>

## 主要特性

- **8 种耦合动力系统** — Logistic、Lorenz、Henon、Rossler、Hindmarsh-Rose、FitzHugh-Nagumo、Kuramoto、Van der Pol
- **12 种单变量 + 2 种多变量替代数据方法 + 自适应选择** — FFT、AAFT、iAAFT、时移、随机重排、周期打乱、孪生替代、相位替代、小幅打乱、截断傅里叶、周期相位 A、周期相位 B；多变量 FFT/iAAFT（保留交叉相关）；`auto` 模式根据频谱集中度自动选择
...
- **替代数据诊断框架** — SSO（替代频谱重叠度）、AUPRC、rho_gap、null_overlap、convergence_fraction 等指标，用于诊断替代数据何时有助于 CCM
- **10 个实验模块** — 双变量验证、耦合强度扫描、噪声鲁棒性、拓扑比较、替代方法比较、鲁棒性消融、诊断表、体制边界扫描、收敛性分析、噪声鲁棒性与周期检测
...
```
seccm/
├── run_experiments.py              # CLI 入口（全部实验）
├── run_diagnostic_experiments.py   # CLI 入口（诊断框架：D1、D2、E5、E7）
├── configs/
│   ├── default.yaml                # 完整实验配置
...
│   │   ├── cycle_phase_surrogate.py    #   周期相位替代（A: 周期内打乱, B: + 周期顺序置换）
│   │   ├── adaptive.py             #   自适应方法选择
│   │   └── multivariate_surrogate.py   #   多变量 FFT/iAAFT 替代
│   ├── testing/                    # 统计检验
│   │   ├── hypothesis_test.py      #   p 值、z 分数、FDR 校正
│   │   └── se_ccm.py              #   SECCM 类（完整流水线，AUROC + AUPRC）
...
│   │   └── van_der_pol.py          #   耦合 Van der Pol 振子
│   ├── evaluation/                 # 检测指标 (TPR, FPR, AUROC, AUPRC, SSO)
│   ├── visualization/              # 绘图工具
│   ├── experiments/                # 实验模块
...
│   │   └── exp_noise_robustness.py     #   E7: 噪声鲁棒性 + 周期检测质量
│   └── utils/                      # I/O、并行执行、混沌检测
```
...
| `noise_robustness` | `--experiment noise_robustness` | E7: 噪声鲁棒性 + 周期检测质量监控 |
...
| **周期相位 A** | 周期内波形 + 周期数 | 周期间相位对齐 | **相位耦合振荡**（VdP、Kuramoto） | 快 |
| **周期相位 B** | 周期内波形 | 周期顺序 + 相位对齐 | **相位耦合振荡**（更强零假设） | 快 |
...
针对**相位耦合振荡系统**（Van der Pol、Kuramoto）设计，在这类系统中周期打乱因周期过于均匀而失效，频谱替代则保留了承载因果信号的周期结构。

**算法：** Hilbert 变换 → 提取瞬时相位 → 在相位 2π 处标记周期边界 → **模式 A：** 在每个周期内循环移位时间点（保留周期数和波形形状）→ **模式 B：** 额外置换周期顺序（更强的零假设）。Hilbert 方法失败时退化为基于峰值的周期检测。
...
surrogate:
  methods: [fft, aaft, iaaft, timeshift, random_reorder, cycle_shuffle, twin,
            phase, small_shuffle, truncated_fourier, cycle_phase_A, cycle_phase_B]
  n_surrogates: 100

surrogate_robustness:
  systems: [logistic, lorenz, henon, rossler, hindmarsh_rose,
            fitzhugh_nagumo, kuramoto, van_der_pol]
  methods: [fft, aaft, iaaft, timeshift, random_reorder, cycle_shuffle, twin,
            phase, small_shuffle, truncated_fourier, cycle_phase_A, cycle_phase_B]
  n_surrogates: 99
  N: 10
...
surrogates = generate_surrogate(data[:, 0], method="cycle_phase_A", n_surrogates=99)  # 周期相位（周期内）
surrogates = generate_surrogate(data[:, 0], method="cycle_phase_B", n_surrogates=99)  # 周期相位（+ 置换顺序）
...
sso = compute_sso(data[:, 0], surrogates)  # 替代频谱重叠度（PSD 的 JSD）
...
seccm.fit(data)
metrics = seccm.score(adj)     # -> 字典：AUROC, AUPRC, TPR, FPR 等
