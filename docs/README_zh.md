...
- **5 种替代数据方法** — FFT、AAFT、iAAFT、时移（time-shift）、随机重排（random reorder）
- **自动嵌入参数选择** — 基于互信息和单纯形预测的数据驱动 E 和 tau 选择
- **网络尺度检验** — 成对 CCM 配合 Benjamini-Hochberg FDR 校正
...
| `surrogate` | `--experiment surrogate` | 比较 5 种替代方法在 7 个系统上的表现 |
...
| Hindmarsh-Rose | ODE（神经元） | 3 | x（膜电位） | I_ext = 3.25, r = 0.006 |
...
| 方法 | 保留 | 破坏 | 速度 |
|---|---|---|---|
| FFT | 功率谱 | 幅度分布、非线性结构 | 快 |
| AAFT | 幅度分布 + 近似功率谱 | 非线性结构 | 快 |
| iAAFT | 幅度分布 + 精确功率谱 | 非线性结构 | 慢（~50x） |
| 时移 | 所有局部结构 | 序列间时间对齐 | 极快 |
| 随机重排 | 幅度分布 | 所有时间结构 | 极快 |
